...
mod core {
    ...
    pub fn init_app(app: AppHandle) -> Result<InitInfo, String> {
        ...
        if created {
            let _ = conn.execute(
                "INSERT OR IGNORE INTO brands(id,name) VALUES(1,'GENÉRICO')",
                [],
            );
        }
    ...
    pub fn get_vehicles_by_make_cmd(
        ...
        if let Some(m) = make.and_then(|s| {
            ...
            for r in rows {
                ...
                if v.name.to_ascii_uppercase().trim() != m {
                    out.push(v);
                }
            }
            Ok(out)
    ...
    fn header_key(s: &str) -> &'static str {
        ...
        if ["FABRICANTE", "MARCA"].contains(&n.as_str()) {
            ...
            "application"
        } else if ["MONTADORA"].contains(&n.as_str()) {
            "ignore"
        ...
        for row in rows {
            ...
            if !vehicles_raw.is_empty() {
                    ...
                    .split(|c| c == ';' || c == ',' || c == '|' || c == '\n' || c == '\r')
                {
                    ...
                    tx.execute("INSERT OR IGNORE INTO vehicles(name) VALUES(?)", params![v])
                        .ok();
                    let vid: i64 = tx
                        ...
                        .unwrap_or_else(|_| 0);
                    if vid != 0 {
                        tx.execute("INSERT OR IGNORE INTO product_vehicles(product_id, vehicle_id) VALUES(?1,?2)", params![pid, vid]).ok();
                        linked += 1;
                    }
                }
            }
