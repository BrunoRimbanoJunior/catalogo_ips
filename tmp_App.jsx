...
function App() {
  ...
  if (!ready) return <main className="container" style={bgPath ? { backgroundImage: `url(${convertFileSrc(bgPath)})`, backgroundSize: "cover", backgroundAttachment: "fixed", backgroundPosition: "center" } : undefined}>CarregandoÔÇª</main>;

  return (
    <main className="container" style={bgPath ? { backgroundImage: `url(${convertFileSrc(bgPath)})`, backgroundSize: "cover", backgroundAttachment: "fixed", backgroundPosition: "center" } : undefined}>
      <div className="appbar">
  <div>{logoPath ? <img className="logo" src={logoSrc} alt="logo" /> : null}</div>
  <h1>Catálogo IPS</h1>
    ...
    {isDev && (<div className="tools"> 
      <details>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, marginTop: 8 }}>
          ...
          <button onClick={async()=>{ const p = await open({ multiple:false, filters:[{ name:'Imagens', extensions:['png','jpg','jpeg','webp'] }]}); if(!p || Array.isArray(p)) return; localStorage.setItem('ui.logoPath', p); setLogoPath(p); setImportMsg('Logo atualizada.'); }}>Selecionar logo…</button>
          <button onClick={async()=>{ const p = await open({ multiple:false, filters:[{ name:'Imagens', extensions:['png','jpg','jpeg','webp'] }]}); if(!p || Array.isArray(p)) return; localStorage.setItem('ui.bgPath', p); setBgPath(p); setImportMsg('Fundo atualizado.'); }}>Selecionar fundo…</button>
